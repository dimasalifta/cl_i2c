...
void Crane_Gripper(String _GLOBAL_GRIP_STR) {
  Serial.println(_GLOBAL_GRIP_STR);
  int cengkraman = _GLOBAL_GRIP_STR.toInt();
  Serial.println(cengkraman);
  if (cengkraman <= 0) {
//    Serial.print("null");
    int map_cengkraman = (cengkraman, 0, 100, 4095, 0);
    pwm.setPWM(solenoidPinA, 0, map_cengkraman);
  }
  else if (cengkraman > 0) {
    int map_cengkramann = (cengkraman, 0, 100, 0, 4095);
    Serial.println(map_cengkramann);
    pwm.setPWM(solenoidPinA, 0, map_cengkramann);
